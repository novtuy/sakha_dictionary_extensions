...
async function getDivFromDict(word) {
  ...
  try {
    ...
    const texts = Array.from(contentBlocks).map((block) =>
  ...
  try {
    ...
    if (data.output.length === 0) {
      ...
      resultsDiv.innerHTML =
        "<p>Көрдөөһүҥҥэ сыыһа баар. Баҕарар VPN холбонон турар эбэтэр интернет арахсан турар</p>";
      return;
    }
...
function applyCase(src, dst) {
  return src === src.toUpperCase() ? dst.toUpperCase() : dst;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isChrome) {
    ...
    api.storage.local.get(["lastWord", "wordProcessed"], (data) => {
      if (data.lastWord && !data.wordProcessed) {
        input.value = data.lastWord;
        doSearch(); // fetch + IndexedDB вызывается только здесь
        api.storage.local.set({ wordProcessed: true });
      }
    });
  }
});
...
document.getElementById("btn1").addEventListener("click", () => {
  api.tabs.create({ url: api.runtime.getURL("flashcards.html") });
});
